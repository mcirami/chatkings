...
{
	if (\LeadMax\TrackYourStats\User\Bonus::updateBonus($assign->id, $_POST["name"], $_POST["salesRequired"], $_POST["payout"], $_POST["status"], isset($_POST["inheritable"]) ? $_POST["inheritable"] : 0)){
        {if (isset($_POST["removeList"]))
	{
		\LeadMax\TrackYourStats\User\Bonus::removeUsersFromBonus($assign->id, $_POST["removeList"]);
	}
}
	if (isset($_POST["replist"]))
	{
		\LeadMax\TrackYourStats\User\Bonus::assignUsersToBonus($assign->id, $_POST["replist"]);
	}
	send_to("bonus_edit.php?id={$assign->id}");
}
}
...
?>
	...
	<div class = "right_panel">
		<div class = "white_box_outer">
				...
				<form action = "<?php htmlspecialchars($_SERVER['PHP_SELF']); ?>" method = "post" id = "form"
					...
					<div class = "left_con01">
						<p>
							...
							<input id = "title" name = "name" type = "text" value = "<?= $result->name ?>" required/>
						</p>
						<p>
							
							<label class = "value_span9">Sales Required</label>
							<input type = "number" name = "salesRequired" value = "<?= $result->sales_required ?>">
						</p>
						
						<p>
							<label class = "value_span9">Payout</label>
							<input type = "number" name = "payout" value = "<?= $result->payout ?>" step = "0.01">
						</p>
						...
						<p>
							...
							<input <?= ($result->inheritable == 1) ? "checked" : ""?> type = "checkbox" class = "fixCheckBox" name = "inheritable" id = "inheritable" value="1">
						</p>
						...
						if (\LeadMax\TrackYourStats\System\Session::permissions()->can("create_admins"))
						{
							...
							<p>
								...
								<select multiple onchange = "moveToSelect(this, 'assignedAdmins', 'unAssignedAdmins')"
										class = "form-control input-sm" id = "assignedAdmins"
										name = "replist[]">
									<?php
								...
								<select multiple onchange = "moveToSelect(this, 'unAssignedAdmins', 'assignedAdmins')"
					...
					<div class = "right_con01">
						...
						if (\LeadMax\TrackYourStats\System\Session::permissions()->can("create_managers"))
						{
							...
							<p>
								...
								<select multiple onchange = "moveToSelect(this, 'assignedManagers', 'unAssignedManagers')"
										class = "form-control input-sm" id = "assignedManagers"
										name = "replist[]">
									<?php
								...
								<select multiple onchange = "moveToSelect(this, 'unAssignedManagers', 'assignedManagers')"
										class = "form-control input-sm "
						...
						if (\LeadMax\TrackYourStats\System\Session::permissions()->can("create_affiliates"))
						{
							...
							<p>
								...
								<select multiple
										onchange = "moveToSelect(this, 'assignedAffiliates', 'unAssignedAffiliates')"
										class = "form-control input-sm" id = "assignedAffiliates"
										name = "replist[]">
									<?php
								...
								<select multiple
										onchange = "moveToSelect(this, 'unAssignedAffiliates', 'assignedAffiliates')"
										class = "form-control input-sm "
			...
			<span class = "btn_yellow"> <input id = "submitBtn" type = "submit" name = "button"
